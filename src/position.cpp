...
namespace Stockfish {
    ...
    std::ostream& operator<<(std::ostream& os, const Position& pos) {
        ...
        const auto* book_entry = Book::find_opening(pos);
        if (book_entry)
            os << "\nPosition: " << book_entry->opening;
    ...
    std::ostream& operator<<(std::ostream& os, Square sq) {
        os << char(file_of(sq) + 'a') << char(rank_of(sq) + '1');
        return os;
    }
    ...
    std::ostream& operator<<(std::ostream& os, PieceType pt) {
        static const char* piece[] = { "","pawn","knight","bishop","rook","queen","king" };
        os << piece[pt];
        return os;
    }

    std::ostream& operator<<(std::ostream& os, Piece p) {
        os << color_of(p) << " " << type_of(p);
        return os;
    }
    ...
    */
        ...
        // Validate FEN string with RegEx.
        {
            std::smatch match;
            std::regex rx(R"(([pnbrqkPNBRQK1-8\/]*)[\s]*([wb])[\s]*(-|K?Q?k?q?|[A-H]?[A-H]?[a-h]?[a-h]?)[\s]*(-|[a-h][1-8])?[\s]*?(-|[\d]*)?[\s]*?([\d]*)?[\s]*)");
            if (!std::regex_match(fenStr, match, rx))
            {
                std::cerr << "ERROR: Not a valid FEN string!" << std::endl;
                return *this;
            }
        }
        ...
        if (((ss >> col) && (col >= 'a' && col <= 'h')) && ((ss >> row) && (row == (sideToMove == WHITE ? '6' : '3'))))
        {
    ...
    // Helper function used to set castling rights given the corresponding color and the rook starting square.
    ...
    // Computes the hash keys of the position, and other data that once computed is updated incrementally as moves are made.
    ...
    // It is mainly a helper to get the material key out of an endgame code.
    Position& Position::set(const std::string& code, Color c, StateInfo* si) {
        ...
        std::string fenStr = "8/" + sides[0] + char(8 - sides[0].length() + '0') + "/8/8/8/8/" + sides[1]
    ...
    // that are blocking attacks on the square 's' from 'sliders'.
    // A piece blocks a slider if removing that piece from the board would result in a position where square 's' is attacked.
    ...
    bool Position::legal(Move m) const {
        ...
        // En passant captures are a tricky special case.
        // Because they are rather uncommon, we do it simply by testing whether the king is attacked after the move is made.
        ...
        {
            // After castling, the rook and king final positions are the same in Chess960 as they would be in standard chess.
    ...
    // It is used to validate moves from TT that can be corrupted due to SMP concurrent access or hash position key aliasing.
    bool Position::pseudo_legal(const Move m) const {
        ...
        // Use a slower but simpler function for uncommon cases yet we skip the legality check of MoveList<LEGAL>().
        if (m.type_of() != NORMAL)
        ...
        // If the 'from' square is not occupied by a piece belonging to the side to move, the move is obviously not legal.
    ...
    bool Position::gives_check(Move m) const {
        ...
        case PROMOTION:
            ...
            // We have already handled the case of direct checks and ordinary discovered check,
            // so the only case we need to handle is the unusual case of a discovered check through the captured pawn.
        case EN_PASSANT:
        {
    ...
    void Position::do_move(Move m, StateInfo& newSt, bool givesCheck) {
        ...
        // except the ones which are going to be recalculated from scratch anyway
        // and then switch our state pointer to point to the new (ready to be updated) state.
        if constexpr (Update)
        ...
        // In particular, rule50 will be reset to zero later on in case of a capture or a pawn move.
        DirtyPiece* dp;
        if constexpr (Update)
        {
            ++gamePly;
        ...
        if (type_of(pc) == PAWN)
        {
            ...
            else if (m.type_of() == PROMOTION)
            {
                ...
                if constexpr (Update)
                {
                    ...
                    st->materialKey ^= Zobrist::psq[promotion][pieceCount[promotion] - 1] ^ Zobrist::psq[pc][pieceCount[pc]];
        ...
        // negative in the 3-fold case, or zero if the position was not repeated.
        if constexpr (Update)
        {
            st->repetition = 0;
    ...
    // Unmakes a move.
    // When it returns, the position should be restored to exactly the same state as before the move was made.
    template<bool Update>
    ...
    // Used to do a "null move": it flips the side to move without executing any move on the board.
    ...
    // Tests if the SEE (Static Exchange Evaluation) value of move is greater or equal to the given threshold.
    // We'll use an algorithm similar to alpha-beta pruning with a null window.
    template <bool Classic>
    bool Position::see_ge(Move m, int threshold) const {
        ...
        int swap = (Classic ? PieceValueME[MG][piece_on(to)] : PieceValue[piece_on(to)]) - threshold;
        if (swap < 0)
            return false;

        swap = (Classic ? PieceValueME[MG][piece_on(from)] : PieceValue[piece_on(from)]) - swap;
        ...
        while (true)
        {
            ...
            // and add to the bitboard 'attackers' any X-ray attackers behind it.
            if ((bb = stmAttackers & pieces(PAWN)))
            {
                if ((swap = (Classic ? PawnValueMg : PawnValue) - swap) < res)
            ...
            else if ((bb = stmAttackers & pieces(QUEEN)))
            {
                ...
                attackers |= (attacks_bb<BISHOP>(to, occupied) & pieces(BISHOP, QUEEN))
                  ...
                  // If we "capture" with the king but the opponent still has attackers, reverse the result.
    ...
    template bool Position::see_ge<false>(Move m, Value threshold) const;
    template bool Position::see_ge<true>(Move m, Value threshold) const;
    ...
    bool Position::is_draw(int ply) const {
        ...
        // Return a draw score if a position repeats once earlier but strictly after the root,
        // or repeats twice before or at the root.
        return st->repetition && st->repetition < ply;
    }


    // Tests whether there has been at least one repetition of positions since the last capture or pawn move.
    ...
    // Performs some consistency checks for the position object and raise an assert if something wrong is detected.
