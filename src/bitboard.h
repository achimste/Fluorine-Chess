...
namespace Stockfish {
    ...
    constexpr bool opposite_colors(Square s1, Square s2) { return (s1 + rank_of(s1) + s2 + rank_of(s2)) & 1; }


    // rank_bb() and file_bb() return a bitboard representing all the squares on the given file or rank.
    ...
    // Returns a bitboard representing an entire line (from board edge to board edge) that intersects the two given squares.
    // If the given square are not on a same file/rank/diagonal, the function returns 0.
    ...
    // front of the given one, from the point of view of the given color.
    // For instance, forward_ranks_bb(BLACK, SQ_D3) will return the 16 squares on ranks 1 and 2.
    ...
    // be attacked by a pawn of the given color when it moves along its file, starting from the given square.
    ...
    // passed_pawn_span() returns a bitboard which can be used to test
    ...
    // distance() functions return the distance between x and y, defined as the number of steps for a king in x to reach y.
    ...
    namespace Pext {
        ...
        constexpr uint64_t Rook(uint64_t square, uint64_t occupy);
        constexpr uint64_t Bishop(uint64_t square, uint64_t occupy);

    }


    // Returns the attacks by the given piece assuming the board is occupied according to the passed Bitboard.
    ...
    inline Square msb(Bitboard b) {
        assert(b);
        return Square(std::countl_zero(b) ^ 0x3f);
    }
    ...
    // frontmost_sq() returns the most advanced square for the given color, requires a non-zero bitboard.
