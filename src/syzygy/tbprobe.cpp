...
namespace Stockfish {
    ...
    namespace {
        ...
        struct LR {
            ...
            uint8_t lr[3]; // The first 12 bits is the left-hand symbol, the second 12 bits is the right-hand symbol.
        ...
        // TBTable is populated at init time but the nested PairsData records are populated at
        ...
        // class TBTables creates and keeps ownership of the TBTable objects, one for each TB file found.
        // It supports a fast, hash-based, table lookup. Populated at init time, accessed at probe time.
        class TBTables {
        ...
        int map_score(TBTable<DTZ>* entry, File f, int value, WDLScore wdl) {
            ...
            // We want to return plies, so we have to convert to plies when needed.
...
// To encode k pieces of the same type and color, first sort the pieces by square in
...
//
        ...
        CLANG_AVX512_BUG_FIX Ret
            ...
            // DTZ tables are one-sided, i.e. they store positions only for white to move or only for black to move,
            // so check for side to move to be stm, early exit otherwise.
            if (!check_dtz_stm(entry, stm, tbFile))
                return *result = CHANGE_STM, Ret();

            // Now we are ready to get all the position pieces (but the lead pawns)
            // and directly map them to the correct color and square.
            ...
            // Then we reorder the pieces to have the same sequence as the one stored in pieces[i]:
            // the sequence that ensures the best compression.
            ...
            // Now we map again the squares so that the square of the lead piece is in the triangle A1-D1-D4.
            if (file_of(squares[0]) > FILE_D)
                for (int i = 0; i < size; ++i)
                    squares[i] = flip_file(squares[i]);

            // Encode leading pawns starting with the one with minimum MapPawns[] and proceeding in ascending order.
            if (entry->hasPawns)
            {
            ...
            // In positions without pawns, we further flip the squares to ensure leading piece is below RANK_5.
            if (rank_of(squares[0]) > RANK_4)
                for (int i = 0; i < size; ++i)
                    squares[i] = flip_rank(squares[i]);

            // Look for the first piece of the leading group not on the A1-D4 diagonal and ensure it is mapped below the diagonal.
            ...
            if (entry->hasUniquePieces)
            {
                ...
                // First piece is below a1-h8 diagonal.
                // MapA1D1D4[] maps the b1-d1-d3 triangle to 0...5.
                // There are 63 squares for second piece and 62 (mapped to 0...61) for the third.
                ...
                // rank_of() maps a1-d4 diagonal to 0...3
                // and finally MapB1H1H7[] maps the b1-h1-h7 triangle to 0..27.
                else if (off_A1H8(squares[1]))
                    idx = (6 * 63 + rank_of(squares[0]) * 28 + MapB1H1H7[squares[1]]) * 62 + squares[2] - adjust2;
        ...
        encode_remaining:
            ...
            while (d->groupLen[++next])
            {
                ...
                // Map down a square if "comes later" than a square in the previous groups
                // (similar to what was done earlier for leading group pieces).
        ...
        // Group together pieces that will be encoded together.
        // The general rule is that a group contains pieces of the same type and color.
        // The exception is the leading group that, in case of positions without pawns,
        // can be formed by 3 different pieces (default) or by the king pair
        // when there is not a unique piece apart from the kings.
        // When there are pawns, pawns are always first in pieces[].
        //
        // As example KRKN -> KRK + N, KNNK -> KK + NN, KPPKP -> P + PP + K + K
        //
        // The actual grouping depends on the TB generator and can be inferred from the sequence of pieces in piece[] array.
        template<typename T>
        void set_groups(T& e, PairsData* d, int order[], File f) {
            ...
            // Number of pieces per group is stored in groupLen[],
            // for instance in KRKN the encoder will default on '111', so groupLen[] will be (3, 1).
            ...
            // The sequence in pieces[] defines the groups, but not the order in which they are encoded.
            // If the pieces in a group g can be combined on the board in N(g) different ways,
            // then the position encoding will be of the form:
            //
            //           g1 * N(g2) * N(g3) + g2 * N(g3) + g3
            //
            // This ensures unique encoding for the whole position.
            // The order of the groups is a per-table parameter and could not follow the canonical leading
            // pawns/pieces -> remaining pawns -> remaining pieces.
            // In particular the first group is at order[0] position
            // and the remaining pawns, when present, are at order[1] position.
        ...
        // In Recursive Pairing each symbol represents a pair of children symbols.
        // So read d->btree[] symbols data and expand each one in his left and right child symbol
        ...
        uint8_t* set_sizes(PairsData* d, uint8_t* data) {
            ...
            // groupLen[] is a zero-terminated list of group lengths,
            // the last groupIdx[] element stores the biggest index that is the tb size.
        ...
        // (winning capture or winning pawn move).
        // Also, DTZ store wrong values for positions where the best move is an ep-move (even if losing).
        // So in all these cases set the state to ZEROING_BEST_MOVE.
        ...
        WDLScore search(Position& pos, ProbeState* result) {
            ...
            for (const Move move : moveList)
            {
                ...
                pos.do_move<true>(move, st);
                value = -search<false>(pos, result);
                pos.undo_move<true>(move);
            ...
            // the TB because the stored score could be wrong.
            ...
            // Also in case of only capture moves, for instance here 4K3/4q3/6p1/2k5/6p1/8/8/8 w - - 0 7,
            // we have to return with ZEROING_BEST_MOVE set.
    ...
    // Called at startup and after every change to "SyzygyPath" UCI option to (re)create the various tables.
    ...
    void Tablebases::init(const std::string& paths) {
        ...
        // MapKK[] encodes all the 462 possible legal positions of two kings where the first is in the a1-d1-d4 triangle.
        ...
        // MapPawns[s] encodes squares a2-h7 to 0..47.
        // This is the number of possible available squares when the leading one is in 's'.
        // Moreover the pawn with highest MapPawns[] is the leading pawn, the one nearest the edge,
        // and among pawns with the same file, the one with the lowest rank.
        int availableSquares = 47; // Available squares when lead pawn is in a2

        // Init the tables for the encoding of leading pawns group: with 7-men TB we can have up to 5 leading pawns (KPPPPPK).
        for (int leadPawnsCnt = 1; leadPawnsCnt <= 5; ++leadPawnsCnt)
            for (File f = FILE_A; f <= FILE_D; ++f)
            {
                // Restart the index at every file because TB table is split by file,
                // so we can reuse the same index for different files.
                int idx = 0;

                // Sum all possible combinations for a given file,
                // starting with the leading pawn on rank 2 and increasing the rank.
                for (Rank r = RANK_2; r <= RANK_7; ++r)
                {
                    Square sq = make_square(f, r);

                    // Compute MapPawns[] at first pass.
                    // If sq is the leading pawn square, any other pawn cannot be below or more toward the edge of sq.
                    // There are 47 available squares when sq = a2
                    // and reduced by 2 for any rank increase due to mirroring: sq == a3 -> no a2, h2, so MapPawns[a3] = 45
                    if (leadPawnsCnt == 1)
                    {
    ...
    int Tablebases::probe_dtz(Position& pos, ProbeState* result) {
        ...
        *result = OK;
        ...
        // DTZ stores results for the other side, so we need to do a 1-ply search and find the winning move that minimizes DTZ.
        StateInfo st;
        int       minDTZ = 0xFFFF;
        ...
        for (const Move move : MoveList<LEGAL>(pos))
        {
            ...
            // otherwise we will get the dtz of the next move sequence.
            // Search the position after the move to get the score sign
            // (because even in a winning position we could make a losing capture or go for a draw).
            ...
            // Convert result from 1-ply search.
            // Zeroing moves are already accounted by dtz_before_zeroing() that returns the DTZ of the previous move.
            if (!zeroing)
                dtz += sign_of(dtz);
    ...
    bool Tablebases::root_probe(Position& pos, Search::RootMoves& rootMoves) {
        ...
        for (auto& m : rootMoves)
        {
            ...
            {
                // In case a root move leads to a draw by repetition or 50-move rule, we set dtz to zero.
                // Note: since we are only 1 ply from the root, this must be a true 3-fold repetition inside the game history.
                dtz = 0;
            }
            else
            {
            ...
            // Determine the score to be displayed for this move.
