...
namespace Stockfish {
            ...
            for (const std::string& directory : dirs)
                if (currentEvalFileName != eval_file)
                {
                    ...
                    if (directory == "<internal>" && eval_file == EvalFileDefaultName)
                    {
                        ...
                        MemoryBuffer buffer(const_cast<char*>
                            (reinterpret_cast<const char*>(gEmbeddedNNUEData)), size_t(gEmbeddedNNUESize));
    ...
    // Returns a static, purely materialistic evaluation of the position from the point of view of the given color.
    // It can be divided by PawnValue to get an approximation of the material advantage on the board in terms of pawns.
    ...
    // trace() is like evaluate(), but instead of returning a value,
    ...
    // Useful for debugging. Trace scores are from white's point of view
...
namespace Stockfish::Classic {
    ...
    namespace Trace {
        ...
        static double to_cp(Value v) { return double(v) / UCI::Classic::NormalizeToPawnValue; }
        ...
        std::ostream& operator<<(std::ostream& os, Score s) {
            os << std::setw(5) << Trace::to_cp(mg_value(s)) << " "
                << std::setw(5) << Trace::to_cp(eg_value(s));
            return os;
        }
...
#undef S
        ...
        public:
            ...
            template<bool SearchMate = false> Value value();

        private:
            template<Color Us> void initialize();
            template<Color Us, PieceType Pt> Score pieces();
            template<Color Us, bool SearchMate> Score king() const;
        ...
        template<Tracing T> template<Color Us, bool SearchMate>
        Score Evaluation<T>::king() const {
            ...
            constexpr int qf = SearchMate ? 473 : 873; // different queen multiplier
            ...
            kingDanger += kingAttackersCount[Them] * kingAttackersWeight[Them] // (~10 Elo)
                ...
                - qf * !pos.count<QUEEN>(Them)                              // (~24 Elo)
            ...
            {
                Bitboard kingSquares = attackedBy[Us][KING] & ~attackedBy[Them][ALL_PIECES] & ~pos.pieces(Us);
                kingDanger -= 100 * popcount(kingSquares);
            }
        ...
        Value Evaluation<T>::winnable(Score score) const {
            ...
            v = mg * int(me->game_phase()) + eg * int(PHASE_MIDGAME - me->game_phase()) * ScaleFactor(sf) / SCALE_FACTOR_NORMAL;
            v /= PHASE_MIDGAME;
        ...
        Value Evaluation<T>::value() {
            ...
            auto lazy_skip = [&](Value lazyThreshold) {
                return std::abs(mg_value(score) + eg_value(score)) > lazyThreshold
                    + std::abs(pos.this_thread()->bestValue) * 5 / 4
                    + pos.non_pawn_material() / 32;
                };

            if (!SearchMate && lazy_skip(LazyThreshold1))
                goto make_v;
            ...
            if constexpr (SearchMate) score = score / 32;

            // More complex interactions that require fully populated attack bitboards
            if constexpr (!SearchMate)
            {
                score += king<WHITE, SearchMate>() - king<BLACK, SearchMate>()
            ...
            {
                score += king<WHITE, SearchMate>() - king<BLACK, SearchMate>();
                score += (threats<WHITE>() - threats<BLACK>()) / 4;
                score += (passed<WHITE>() - passed<BLACK>()) / 2;
            }
        ...
        make_v:
            ...
            {
                static_assert(T == NO_TRACE, "Trace not allowed in SearchMate");
                ...
                v = (mg * int(me->game_phase()) + eg * int(PHASE_MIDGAME - me->game_phase())) / PHASE_MIDGAME;
            }
    ...
    Value Eval::evaluate(const Position& pos, int* complexity) {
        ...
        Value v = Evaluation<NO_TRACE>(pos).value<SearchMate>();
        ...
        if (complexity)
            *complexity = std::abs(v - pos.psq_eg_stm());

        return v;
    }

    template Value Eval::evaluate<false>(const Position& pos, int* complexity);
    template Value Eval::evaluate<true>(const Position& pos, int* complexity);
    ...
    std::string Eval::trace(Position& pos) {
        ...
        v = evaluate<false>(pos);
        v = pos.side_to_move() == WHITE ? v : -v;
        ss << "Final evaluation       " << Trace::to_cp(v) << " (white side)";
        ss << "\n";
